...
#endif
{
    ...
    if (SaveLog::Instance()->getUseContext()) {
        ...
        if (line > 0) {
            content = QString("[%1][%2]%3[%4] >> %5").arg(TIMEMS).arg(content).arg(file).arg(line).arg(msg);
        }
    }
#endif
